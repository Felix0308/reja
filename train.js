...
class Shop {
  ...
  logTime() {
    const now = new Date();
    return `hozir ${now.getHours()}:${now.getMinutes() < 10 ? '0' : ''}${now.getMinutes()}da`;
  }

  qoldiq() {
    const { non, lagmon, cola } = this.products;
    console.log(
      `${this.logTime()} ${non}ta non, ${lagmon}ta lagmon va ${cola}ta cola mavjud!`
    );
    return { ...this.products };
  }

  sotish(tur, miqdor) {
    if (!this.products[tur]) {
      console.log(`${this.logTime()} ${tur} mahsuloti mavjud emas!`);
      return;
    }

    if (this.products[tur] < miqdor) {
      console.log(`${this.logTime()} yetarli ${tur} mahsuloti yo'q!`);
      return;
    }

    this.products[tur] -= miqdor;
    console.log(`${this.logTime()} ${miqdor}ta ${tur} sotildi!`);
  }

  qabul(tur, miqdor) {
    if (!this.products[tur]) {
      console.log(`${this.logTime()} ${tur} mahsuloti do'konda yo'q!`);
      return;
    }

    this.products[tur] += miqdor;
    console.log(`${this.logTime()} ${miqdor}ta ${tur} qabul qilindi!`);
  }
}
...
shop.qoldiq();           // Masalan: hozir 20:40da 4ta non, 5ta lagmon va 2ta cola mavjud!
shop.sotish("non", 3);   // Masalan: hozir 20:42da 3ta non sotildi!
shop.qabul("cola", 4);   // Masalan: hozir 20:45da 4ta cola qabul qilindi!
shop.qoldiq();           // Masalan: hozir 20:50da 1ta non, 5ta lagmon va 6ta cola mavjud!